...
This repo only stands up the lab itself (the two BMC-managed nodes, `KUBECONFIG`,
and the running metal-operator/boot-operator/FeDHCP/TFTP stack) — it does not
contain any tests. To run the [metal-operator-test-framework](https://github.com/simontesar/metal-operator-test-framework)
compatibility suite against this lab, check out that repo separately and
